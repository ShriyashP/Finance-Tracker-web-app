...
export default function FinanceContextProvider({ children }) {
  ...
  const deleteExpensesItem = async (updatedExpenses, expenseCategoryId) => {
    try {
      ...
      setExpenses((prevState) => {
        const updatedExpensesList = [...prevState];
        const pos = updatedExpensesList.findIndex((ex) => ex.id === expenseCategoryId);
        
        // Update the specific expense category with the new items and total
        updatedExpensesList[pos] = {
          ...updatedExpensesList[pos],
          items: updatedExpenses.items,
          total: updatedExpenses.total
        };
